...
yss.helper('button', (y, background) => y.flex.bn.round.ttu.i.fs1.pv3.ph4`
  color white
...
const responsiveDiv = 'div' + yss.size(5).bg('red').media('(max-width: 1000px)', yss.bg('green'))
...
const zebraDiv = 'div' + yss.size(5).animate('10s infinite', bwAnimation)`transition background 1s`
const discoDiv = 'div' + yss.size(5).animate('9s infinite', rainbow)`transition background 1s`
const anotherDiscoDiv = 'div' + yss.size(5).animate('10s infinite', rainbow)`transition background 1s`
...
m.mount(document.body, {
  ...
  view({state}) {
    return [
      m('style', yss.css),
      m(state.on ? greenButton : blueButton, {
