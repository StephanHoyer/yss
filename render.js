...
function toCssDefinition(cssClass, style) {
  ...
  const mediaStyles = keys(style)
    ...
    .join('')
  const keyframes = map(animationNameByStyle, (name, keyframeStyle) => `@keyframes ${name}{${keyframeStyle}}`).join('')
  return `${keyframes}${cssClass}{${baseStyle}}${subStyles}${mediaStyles}`
}
